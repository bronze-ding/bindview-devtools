...
> | 依赖 | 最低版本 | 说明 |
> | --- | --- | --- |
> | **bindview** | **`3.2.0`** | 框架侧需包含 DevTools 集成(生命周期通知、方法调用上报、`_linkage` / `_DataProxy` 等接口) |
> | **bindview-router** | **`1.2.0`** | 可选;使用「路由」页签时需要(路由状态 / 层级 / 路由表上报) |
> | Chromium 内核 | `111` | `world: "MAIN"` 内容脚本与 `scripting` 兜底探测需要 |
>
> 低于最低版本时面板仍可用,但**新能力会降级**(见「常见问题 → 组件列表为空」)。

---

## 一、安装

支持 Chrome / Edge / Brave 等 Chromium 内核浏览器。完整环境要求见上方提示框:内核 ≥ **111**、
**bindview ≥ `3.2.0`**、**bindview-router ≥ `1.2.0`**(可选,「路由」页签需要)。
...
**最低支持版本:bindview `3.2.0`,bindview-router `1.2.0`。**
...
2. 改了应用 / `bindview` / `bindview-router` 代码 → **刷新目标页面**。
...
**组件列表为空**
: 确认框架侧已完成第四节集成 —— 需要 **bindview ≥ `3.2.0`**(版本内包含 `tools/devtools.js`),或点击右上角「刷新」重新扫描。

**框架版本低于最低要求会怎样**
: 最低要求为 **bindview ≥ `3.2.0`**、**bindview-router ≥ `1.2.0`**。低于该版本时面板整体仍可用,但会**优雅降级**:

- 组件树、`props`/`data`/`refs` 检查、状态编辑、事件时间线、高亮等基础能力不受影响(这些只依赖 `__BINDVIEW_DEVTOOLS_GLOBAL_HOOK__`);
- **页面内方法调用不会记入时间线**(需要 `3.2.0` 的方法包装上报),此时仅面板发起的调用有记录;
- 组件树上**不显示 `linkage:false` 徽标**(需要 `_linkage` 字段);
- 「初始化 data」不可用(需要框架暴露 `_DataProxy`);
- 「路由」页签需要 **bindview-router ≥ `1.2.0`**;更低版本会提示未检测到路由插件。
